...
num_epochs : integer
  ...
  safely interrupt training with Ctrl+C at any time.'''
        workbook = xlsxwriter.Workbook(self.__class__.__name__ + str(num_epochs) + '.xlsx')
            ...
            for epoch in xrange(num_epochs):
                ...
                for s, sequence in enumerate(dataset):
                    for i in xrange(0, len(sequence), batch_size):
                        ...
                        costs1.append(cost1)
                        print('Epoch %i: sequence %i: %i' % (epoch + 1, s, i))
                        print('Cross-entropy: %f' % cost1)
                numcost = numpy.mean(costs1)
                worksheet.write('A' + str(epoch + 1), numcost)
                print('Epoch %i. Cross-entropy: %f' % ((epoch + 1), numcost))
                print_to_file('Epoch %i: %f' % ((epoch + 1), numcost), 'log.txt')
              
                print('Generating training%i.mid' % (epoch+1))
                self.generate('training' + str(epoch + 1) + '.mid')
                print('Successfully generating training%i.mid' % (epoch + 1))
                sys.stdout.flush()
  ...
  safely interrupt training with Ctrl+C at any time.'''
        ...
        try:
            for epoch in xrange(num_epochs):
                ...
                for s, sequence in enumerate(dataset):
                    for i in xrange(0, len(sequence), batch_size):
                        cost2 = self.train_function1(sequence[i:i + batch_size])
                        costs2.append(cost2)
                        print('Epoch %i: sequence %i: %i' % (epoch + 1, s, i))
                        print('Cross-entropy: %f' % cost2)
                numcost = numpy.mean(costs2)
                worksheet.write('B' + str(epoch + 1), numcost)
                print('Epoch %i. Cross-entropy: %f' % ((epoch + 1), numcost))
                print_to_file('Epoch %i: %f' % ((epoch + 1), numcost), 'log.txt')
...
def test_rnnrbm(batch_size=100, num_epochs=200):
    ...
    re = os.path.join(os.path.split(os.path.dirname(__file__))[0], 'data', 'scorpions', '*.mid')
