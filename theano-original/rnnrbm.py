...
class RnnRbm:
    ...
    def printToFile(self, where, what):
        file = open(where, 'a')
        file.write(what)
        file.write("\n");
        file.close()
    ...
    def train(self, files, batch_size=100, num_epochs=200):
        ...
        self.printToFile('Training', 'log.txt')
        assert len(files) > 0, 'Training set is empty!' \
                               ' (did you download the data files?)'
        dataset = [midiread(f, self.r,
                            self.dt).piano_roll.astype(theano.config.floatX)
                   for f in files]
        self.printToFile('dataset', 'log.txt')
        self.printToFile(str(dataset), 'log.txt')
    ...
    #                         costs.append(cost)
    # 			print('%i => %i' % (i, cost))
    # 			self.printToFile('%i => %i' % (i, cost), 'log.txt')
    #
    #                 print('Epoch %i/%i' % (epoch + 1, num_epochs))
    # 		self.printToFile('Epoch %i/%i' % (epoch + 1, num_epochs), 'log.txt')
    # 		numpycostss = numpy.mean(costs)
    #                 print(numpycostss)
    # 		self.printToFile(str(numpycostss), 'log.txt')
    # 		print('Generating training%i.mid' % (epoch + 1))
    # 		self.printToFile('Generating training%i.mid' % (epoch + 1), 'log.txt')
    # 		self.generate('training' + str(epoch) + '.mid', false)
    # 		print('Successful generating to training%i.mid' % (epoch + 1))
    # 		self.printToFile('Successful generating to training%i.mid' % (epoch + 1), 'log.txt')
    #                 sys.stdout.flush()

    # picklestring = pickle.dumps(self)
    # self.printToFile(str(picklestring), 'dump.txt')
    ...
    def generate(self, filename, show=True):
        ...
        if show:
            ...
            pylab.figure()
            pylab.imshow(piano_roll.T, origin='lower', aspect='auto', interpolation='nearest', cmap=pylab.cm.gray_r,
                         extent=extent)
            pylab.xlabel('time (s)')
